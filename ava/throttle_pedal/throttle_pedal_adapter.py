...
dbw_mount_depth = 20
dbw_lower_left_hole_position = (-30, -40)
dbw_hole_spacing = (56.61, 77.33)
dbw_hole_dia = 6  # TODO: set up for insert
dbw_hole_countersink_dia = 8
# TODO: we may want to angle the support plate
# TODO: derived?
dbw_mount_dimensions = (85, 105)
...
result = (
    ...
    .faces(">Z")
    .workplane(origin=dbw_mount_position)
    .rect(*dbw_mount_dimensions)
...
# chamfer plate outer corners
result = result.faces("<Y or >Y or <X or >X").edges("|Z").chamfer(corner_chamfer)

# chamfer underside of plate
result = (
    result.faces(">Z[1]")
    .edges(cq.selectors.RadiusNthSelector(0, directionMax=False))
    .fillet(underside_chamfer)
    .faces(">Z[1]")
    .edges("not %CIRCLE")
    .chamfer(underside_chamfer)
)

# countersink dbw mount holes
result = result.faces(">Z").edges("%CIRCLE").chamfer((dbw_hole_countersink_dia - dbw_hole_dia) / 2)
